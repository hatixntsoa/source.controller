...
usage() {
	...
	echo "  This script switches to the previously"
	echo "  checked-out branch in a Git repository."
	echo "  It checks if the current directory"
	echo "  is a Git repository, and performs the branch switch."
	echo
	echo "${BOLD}Options:${RESET}"
	echo "  --help            Display this help message."
	echo
	echo "  No arguments means to switch to the last checked-out branch."
	exit 0
}
...
if [ "$is_a_git_repo" = "true" ]; then
	if [ $# -eq 0 ]; then
		...
		echo "${BOLD}${WHITE} ■■▶ Usage : gcb (no argument)"
	fi
else
